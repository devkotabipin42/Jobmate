                    ...
                    <nav className='flex-1 p-3 space-y-1 overflow-y-auto'>
                        {sidebarItems.map(item => (
                            <button key={item.id} onClick={() => handleTabChange(item.id)}
                                ...
                                {item.id === 'documents' && pendingDocuments.length > 0 && (
                                    <span className='ml-auto text-xs bg-red-500 text-white w-5 h-5 rounded-full flex items-center justify-center font-bold'>
                                        {pendingDocuments.length}
                                    </span>
                                )}
                                {item.id === 'contact-requests' && contactRequests.filter(r => r.status === 'pending').length > 0 && (
                                    <span className='ml-auto text-xs bg-blue-500 text-white w-5 h-5 rounded-full flex items-center justify-center font-bold'>
    ...
    <AdminEmployers key='employers' employers={employers} loading={loading}
        ...
        setEmployers={setEmployers} />
                        ...
                        {activeTab === 'contact-requests' && (
                            <AdminContactRequests key='contact-requests'
                                requests={contactRequests}
                                setRequests={setContactRequests}
                                reviewContactRequest={reviewContactRequest}
                                loading={loading} />
                        )}
                    </AnimatePresence>
                </main>
