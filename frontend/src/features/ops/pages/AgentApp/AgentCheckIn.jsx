...
const AgentCheckIn = () => {
    ...
    const canContinue = isFollowUpTask || !!location
        ...
        <div className='min-h-screen bg-gray-50 dark:bg-gray-900'>
            <div className='bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center gap-3'>
                ...
                <div>
                    <p className='text-xs text-gray-500'>
                        {isFollowUpTask ? 'Follow-up Task' : 'Check-in'}
                    </p>
            ...
            <div className='p-4 space-y-4'>
                <div className='bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700'>
                    <p className='text-xs text-gray-500 dark:text-gray-400'>Address / Target</p>
                ...
                {isFollowUpTask ? (
                    <div className='bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700'>
                        ...
                        <p className='text-sm text-gray-700 dark:text-gray-300'>
                            This is a phone/WhatsApp follow-up. GPS is optional. You can continue directly and submit the follow-up result.
                        </p>

                        <button
                            onClick={getLocation}
                            disabled={gettingGps}
                            className='w-full py-3 bg-gray-700 dark:bg-gray-700 text-white rounded-lg text-sm disabled:opacity-50 mt-4'
                        >
                            {gettingGps ? 'Fetching location...' : 'Optional: Capture current location'}
                        </button>

                        {gpsError && <p className='text-xs text-red-600 mt-2'>{gpsError}</p>}

                        {location && (
                            <div className='p-3 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-300 mt-3'>
                    ...
                    <div className='bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700'>
                        ...
                        {!location && (
                            <button
                                onClick={getLocation}
                                disabled={gettingGps}
                                className='w-full py-3 bg-blue-600 text-white rounded-lg text-sm disabled:opacity-50'
                            >
                        ...
                        {location && (
                            <div className={`p-3 rounded-lg ${gpsOk ? 'bg-green-50 dark:bg-green-900/20 border border-green-300' : 'bg-amber-50 dark:bg-amber-900/20 border border-amber-300'}`}>
                                ...
                                {noTargetGps ? (
                                    <p className='text-xs text-gray-600 dark:text-gray-400 mt-1'>
                                        This GPS will be saved as the business location. GPS accuracy: {Math.round(location.accuracy)}m
                                    </p>
                                ) : (
                                    <p className='text-xs text-gray-600 dark:text-gray-400 mt-1'>
                                        Distance from target: {dist}m · GPS accuracy: {Math.round(location.accuracy)}m
                                    </p>
                                )}

                                {!gpsOk && !noTargetGps && (
                                    <p className='text-xs text-amber-700 dark:text-amber-300 mt-2'>
                                        Please move closer to the target location.
                                    </p>
                                )}

                                <button onClick={getLocation} className='text-xs text-blue-600 mt-2'>
                                    Retry
                                </button>
                            </div>
                        )}
                ...
                <button
                    ...
                    className='w-full py-3 bg-blue-600 text-white rounded-lg text-sm font-semibold disabled:opacity-40'
                >
                    {isFollowUpTask
                        ? 'Continue to follow-up result'
                        : gpsOk
                            ? 'Continue to details'
                            : location
                                ? 'Continue anyway'
                ...
                <p className='text-center text-xs text-gray-500'>
                    Data will save offline and sync when online
                </p>
            </div>
        </div>
    )
